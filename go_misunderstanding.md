...
如果访问了 map 中不存在的 key , Go 则会返回元素对应数据类型的零值，比如 `nil`、`''` 、`false` 和 0，取值操作总有值返回，故不能通过取出来的值来判断 key 是不是在 map 中。
...
`下面 main()` 主程序不等两个 goroutine 执行完就直接退出了：
...
在 C++ 中使用 `new` 创建的变量总是分配到堆内存上的，但在 Go 中即使使用 `new()`、`make()` 来创建变量，变量的内存分配位置依旧归 Go 编译器管。
